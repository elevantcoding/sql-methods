...
CREATE FUNCTION [elevant].[CipherString](@string varchar(128), @vals varchar(6), @randval tinyint, @ciph varchar(10), @padding varchar(128))
RETURNS varchar(256)
AS
BEGIN
	DECLARE @strLen int;
...
CREATE FUNCTION [elevant].[DecipherString](@cipherstring varchar(256))
RETURNS varchar(128)
AS
BEGIN
	DECLARE @prefixlen int;
...
CREATE PROC [elevant].[GetCipherString](@stringtocipher varchar(128), @ciphered varchar(256) OUT)
AS
BEGIN
	-- generate:
